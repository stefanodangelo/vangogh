...
def read_single_mask(path, hard_width): 
    ...
    rawmask = rawmask.astype(np.float64)   
...
def read_colorful_mask(target_path, style_path, hard_width, n_colors):
    ...
    # predict
    target_labels = kmeans.predict(target_mask.astype(np.float64))
    target_labels = target_labels.reshape([target_shape[0], target_shape[1]])
    style_labels = kmeans.predict(style_mask.astype(np.float64))
    ...
    for i in range(n_colors):
        target_masks.append( (target_labels == i).astype(np.float64) )
        style_masks.append( (style_labels == i).astype(np.float64) )
...
def get_init_image(content_img, init_noise_ratio):
    # why [-20, 20]???
    noise_img = np.random.uniform(-20., 20., content_img.shape).astype(np.float64)
